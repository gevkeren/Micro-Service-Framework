...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class, Callback> typeToCallback;//hashmap that connects between the event and its callback
    protected MessageBusImpl MB;
    ...
    public MicroService(String name) {
        this.name = name;
        typeToCallback = new ConcurrentHashMap<>();
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        typeToCallback.putIfAbsent(type, callback);
    ...
    public final void run() {
        ...
        while (!terminated) {//while not terminate train the models and when finish get the new one
            //we need to set the model into the micro service
            try {
                Message msg = awaitMessage(this);
                Callback callback = typeToCallback.get(msg.getClass());
                callback.call(msg);
            } catch (InterruptedException e) {
                terminate();
            }
...
//        System.out.println("Micro service " + name + " terminated");
        MB.unregister(this);
    ...
    public ConcurrentHashMap<Class, Callback> getTypeToCallback() {
        return typeToCallback;
    }
